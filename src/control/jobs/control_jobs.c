...
static int32_t dt_control_print_job_run(dt_job_t *job)
{
  ...
  g_strlcpy(filename, pp->filename, PATH_MAX);
  g_strlcat(filename, ".tif", PATH_MAX);
  ...
  if (strlen(pp->pinfo.printer.profile)>0 && g_file_test(pp->pinfo.printer.profile,G_FILE_TEST_EXISTS))
  {
    ...
    {
      dt_control_log(_("failed to apply printer profile '%'"), pp->pinfo.printer.profile);
      return 1;
    }
  }
  ...
  char tag[256] = { 0 };
  guint tagid = 0;
  snprintf (tag, 256, "darktable|printed|%s", pp->pinfo.printer.name);
                      ...
                      char *style, const char *filename, const dt_print_info_t *pinfo)
{
  ...
  data->format_index = format_index;
  data->storage_index = storage_index;
  g_strlcpy(data->filename, filename, PATH_MAX);
